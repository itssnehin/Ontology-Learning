    ...
    def create_component_analysis(self):
        ...
        with self.driver.session() as session:
            ...
            cat_stats = session.run("""
                MATCH (family:OntologyClass)-[:SUBCLASS_OF]->(:OntologyClass {name: 'ElectronicComponent'})
                // Optional: Count how many concepts exist under each family
                OPTIONAL MATCH (c)-[:SUBCLASS_OF*]->(family)
                RETURN family.name as Category, count(c) as Count
                ORDER BY Count DESC
            """)
